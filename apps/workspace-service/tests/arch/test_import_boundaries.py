"""Architecture tests: verify import boundaries for workspace-service.
...
SERVICE_ROOT = Path(__file__).resolve().parents[2]  # apps/workspace-service
...
def test_workspace_repository_adapter_implements_port():
    """WorkspaceRepository adapter in infrastructure must implement the port."""
    adapter_file = INFRA_DIR / "db" / "workspace_repository.py"
    assert adapter_file.exists(), "infrastructure/db/workspace_repository.py not found"

    source = adapter_file.read_text(encoding="utf-8")
    assert "WorkspaceRepository" in source, "Must reference WorkspaceRepository port"
...
def test_outbox_writer_adapter_implements_port():
    """Outbox adapter in infrastructure must implement OutboxWriter port."""
    adapter_file = INFRA_DIR / "outbox" / "outbox_repository.py"
    assert adapter_file.exists(), "infrastructure/outbox/outbox_repository.py not found"

    source = adapter_file.read_text(encoding="utf-8")
    assert "OutboxWriter" in source, "Must reference OutboxWriter port"
