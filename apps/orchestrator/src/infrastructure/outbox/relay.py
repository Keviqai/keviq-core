"""Outbox relay — polls orchestrator outbox and forwards events to event-store.

Phase C: HTTP-based relay with connection reuse and retry.
The relay client is created once and reused across relay cycles.
"""
...
# Module-level shared client — initialised lazily by get_relay_client().
...
def relay_pending_events(session: Session) -> int:
    ...
    """
    # Fetch unpublished events, skip poison-pill rows
    rows = (
    ...
    # Single batch commit instead of per-row
    session.commit()
