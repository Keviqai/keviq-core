"""Architecture tests: verify import boundaries for api-gateway.

Ensures clean-architecture layering:
- Application layer has zero infrastructure imports (no jwt, httpx, sqlalchemy, etc.)
...
Note: api-gateway has a minimal domain layer (mostly routing logic lives in
application), so we focus on application-layer purity and adapter contracts.
"""
...
SERVICE_ROOT = Path(__file__).resolve().parents[2]  # apps/api-gateway
...
INFRA_PACKAGES = {
    "sqlalchemy", "psycopg2", "httpx", "bcrypt", "jwt",
    "uvicorn",
}
...
def test_domain_does_not_import_infrastructure():
    """Domain layer must not import any infrastructure package."""
    violations: list[str] = []
...
def test_application_does_not_import_infrastructure_packages():
    """Application layer must not import jwt, httpx, sqlalchemy, etc."""
    violations: list[str] = []
...
def test_jwt_verifier_adapter_implements_port():
    """JwtVerifierAdapter in infrastructure must implement JwtVerifier port."""
    adapter_file = INFRA_DIR / "jwt_verifier.py"
    assert adapter_file.exists(), "infrastructure/jwt_verifier.py not found"

    source = adapter_file.read_text(encoding="utf-8")
    assert "JwtVerifierAdapter" in source, "JwtVerifierAdapter class not found"
    assert "JwtVerifier" in source, "Must reference JwtVerifier port"
...
def test_service_proxy_adapter_implements_port():
    """ServiceProxyAdapter in infrastructure must implement ServiceProxy port."""
    adapter_file = INFRA_DIR / "service_proxy.py"
    assert adapter_file.exists(), "infrastructure/service_proxy.py not found"

    source = adapter_file.read_text(encoding="utf-8")
    assert "ServiceProxyAdapter" in source, "ServiceProxyAdapter class not found"
    assert "ServiceProxy" in source, "Must reference ServiceProxy port"
...
def test_policy_client_adapter_implements_port():
    """PolicyClient adapter in infrastructure must implement the port."""
    adapter_file = INFRA_DIR / "policy_client.py"
    assert adapter_file.exists(), "infrastructure/policy_client.py not found"

    source = adapter_file.read_text(encoding="utf-8")
    assert "PolicyClient" in source, "Must reference PolicyClient port"
...
def test_workspace_client_adapter_implements_port():
    """WorkspaceClient adapter in infrastructure must implement the port."""
    adapter_file = INFRA_DIR / "workspace_client.py"
    assert adapter_file.exists(), "infrastructure/workspace_client.py not found"

    source = adapter_file.read_text(encoding="utf-8")
    assert "WorkspaceClient" in source, "Must reference WorkspaceClient port"
